<section class="w-full px-4 lg:px-6 mb-8">
    <div class="w-full">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            ...
            <a href="{{ route('shop') }}?price={{ $price }}" class="relative bg-[#2f5cc4] rounded-[24px] aspect-[3/2] flex flex-col justify-center items-center overflow-hidden p-6 group cursor-pointer shadow-sm hover:shadow-md transition-shadow">
                ...
                <div class="relative z-10 flex flex-col items-center text-center space-y-2">
                    <span class="text-white text-xs md:text-sm font-extrabold uppercase tracking-widest opacity-95">UNDER</span>
                    <h3 class="text-white text-3xl md:text-5xl font-black leading-none pb-2">₹{{ number_format($price) }}</h3>
                    <button class="bg-white text-gray-900 text-[11px] font-bold px-6 py-2 rounded-full shadow-sm hover:scale-105 transition-transform duration-200">Explore Now</button>
                </div>
            </a>
            @endforeach
        </div>
