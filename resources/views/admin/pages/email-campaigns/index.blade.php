...
<div class="grid grid-cols-2 md:grid-cols-5 gap-4">
    @foreach([
        ['id'=>'sTot',  'label'=>'Total',      'color'=>'blue',   'icon'=>'M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z'],
        ['id'=>'sSent', 'label'=>'Sent',        'color'=>'green',  'icon'=>'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z'],
        ['id'=>'sSch',  'label'=>'Scheduled',   'color'=>'yellow', 'icon'=>'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z'],
        ['id'=>'sDraft','label'=>'Draft',        'color'=>'gray',   'icon'=>'M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z'],
        ['id'=>'sOpen', 'label'=>'Avg Open Rate','color'=>'purple', 'icon'=>'M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z'],
    ] as $s)
...
async function save(){
    ...
    if(data.success){showSuccessDialog(id?'Campaign updated':'Campaign created');closeModal();load(1);}
    else{const e=data.errors?Object.values(data.errors)[0][0]:data.message;toast(e||'Error','error');}
}

// ── Actions ──────────────────────────────────────────────────────
async function del(id,name){
    showConfirmDialog('Delete Campaign',`Delete "<strong>${name}</strong>"?<br><span class="text-xs text-red-500">This cannot be undone.</span>`,
        async()=>{const data=await api(`${BASE}/${id}`,'DELETE');if(data.success){showSuccessDialog('Campaign deleted');load(1);}else toast(data.message||'Error','error');});
}
...
async function applyBulk(){
    ...
    const doIt=async()=>{const data=await api(`${BASE}/bulk-action`,'POST',{action,ids:[...checkedIds]});if(data.success){if(action==='delete')showSuccessDialog(`${checkedIds.size} deleted`);else toast(data.message);clearSel();load(1);}else toast(data.message||'Error','error');};
    if(action==='delete')showConfirmDialog('Delete Campaigns',`Delete ${checkedIds.size} campaign(s)?`,doIt);else doIt();
}

// ── Dialogs ──────────────────────────────────────────────────────
function showConfirmDialog(title,message,onConfirm){
    const d=document.createElement('div');d.className='fixed inset-0 z-[9999] flex items-center justify-center p-4';
    d.innerHTML=`<style>@keyframes dlg-backdrop{from{opacity:0}to{opacity:1}}@keyframes dlg-pop{0%{opacity:0;transform:scale(0.85) translateY(16px)}60%{opacity:1;transform:scale(1.03) translateY(-3px)}80%{transform:scale(0.98) translateY(1px)}100%{opacity:1;transform:scale(1) translateY(0)}}.dlg-bg{animation:dlg-backdrop .2s ease forwards}.dlg-box{animation:dlg-pop .35s cubic-bezier(0.34,1.56,0.64,1) forwards}</style>
        ...
        <div class="dlg-box relative bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6">
            <div class="flex flex-col items-center text-center gap-3 mb-5"><div class="w-14 h-14 bg-red-50 rounded-full flex items-center justify-center"><svg class="w-7 h-7 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></div>
            <div><h3 class="text-base font-semibold text-gray-900">${title}</h3><p class="text-sm text-gray-500 mt-1 leading-relaxed">${message}</p></div></div>
            <div class="flex gap-2.5"><button onclick="this.closest('.fixed').remove()" class="flex-1 px-4 py-2.5 bg-gray-100 text-gray-700 text-sm font-medium rounded-xl hover:bg-gray-200 transition-colors">Cancel</button><button id="_cok" class="flex-1 px-4 py-2.5 bg-red-500 text-white text-sm font-semibold rounded-xl hover:bg-red-600 transition-colors">Delete</button></div>
        </div>`;
    document.body.appendChild(d);d.querySelector('#_cok').addEventListener('click',()=>{d.remove();onConfirm();});
}
