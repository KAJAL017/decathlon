...
<div class="grid grid-cols-2 md:grid-cols-5 gap-4">
    @foreach([
        ['id'=>'sTotal',     'label'=>'Total',     'color'=>'blue',   'icon'=>'M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z'],
        ...
        ['id'=>'sUsed',      'label'=>'Total Used', 'color'=>'purple','icon'=>'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z'],
    ] as $s)
...
async function save(){
    ...
    if(data.success){showSuccessDialog(id?'Coupon updated':'Coupon created');closeModal();load(1);}
    else{const e=data.errors?Object.values(data.errors)[0][0]:data.message;toast(e||'Error','error');}
}

// ── Delete / Toggle / Bulk ───────────────────────────────────────
async function del(id,code){
    showConfirmDialog('Delete Coupon',`Delete coupon <strong class="font-mono">${code}</strong>?<br><span class="text-xs text-red-500">This cannot be undone.</span>`,
        async()=>{const data=await api(`${BASE}/${id}`,'DELETE');if(data.success){showSuccessDialog('Coupon deleted');load(1);}else toast(data.message||'Error','error');});
}
...
async function applyBulk(){
    ...
    if(action==='delete')showConfirmDialog('Delete Coupons',`Delete ${checkedIds.size} coupon(s)?`,doIt);else doIt();
}
