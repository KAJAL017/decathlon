...
<div id="confirmDialog" class="hidden fixed inset-0 z-[60] flex items-center justify-center p-4">
    ...
    <div id="confirmBox" class="relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm" style="transform:scale(0.8) translateY(20px);opacity:0;transition:all .3s cubic-bezier(.34,1.56,.64,1)">
        ...
        <h3 class="text-lg font-bold text-gray-900 text-center mb-1">Delete Review</h3>
...
function showConfirmDialog(msg, cb) {
    ...
    requestAnimationFrame(() => { const b = document.getElementById('confirmBox'); b.style.transform = 'scale(1) translateY(0)'; b.style.opacity = '1'; });
    document.getElementById('confirmOkBtn').onclick = () => { closeConfirm(); cb(); };
}
function closeConfirm() {
    const b = document.getElementById('confirmBox'); b.style.transform = 'scale(0.8) translateY(20px)'; b.style.opacity = '0';
...
function showSuccessDialog(msg) {
    ...
    requestAnimationFrame(() => {
        const b = document.getElementById('successBox'); b.style.transform = 'scale(1) translateY(0)'; b.style.opacity = '1';
        setTimeout(() => { document.getElementById('checkPath').style.strokeDashoffset = '0'; }, 50);
    });
    setTimeout(() => {
        const b = document.getElementById('successBox'); b.style.transform = 'scale(0.8) translateY(20px)'; b.style.opacity = '0';
        setTimeout(() => { document.getElementById('successDialog').classList.add('hidden'); document.getElementById('checkPath').style.strokeDashoffset = '40'; }, 300);
    }, 2000);
}
